trigger Sales_MarketingFieldPreventer on Sales_Marketing__c (before insert, before update) {
     Map<String, Sales_Marketing__c > leadMap = new Map<String, Sales_Marketing__c >();
    for (Sales_Marketing__c  lead : System.Trigger.new) {          
    ...
    for (Sales_Marketing__c  lead : [SELECT Month__c, Year__c FROM Sales_Marketing__c  
                      WHERE Month__c IN :leadMap.KeySet() and Year__c IN :leadMap.KeySet()]) {
        Sales_Marketing__c  newLead = leadMap.get(lead.Month__c);
        newLead.Month__c.addError('A Sales_Marketing Data Form with this Month'
                               + ' already exists.');
        Sales_Marketing__c  newLead2 = leadMap.get(lead.Year__c);
