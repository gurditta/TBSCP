trigger FinanceFieldPreventer on Finance_Dataform__c(before insert, before update) {
    ...
    for (Finance_Dataform__c lead : [SELECT Month__c, Year__c FROM Finance_Dataform__c 
                      WHERE Month__c IN :leadMap.KeySet() and Year__c IN :leadMap.KeySet()]) {
        Finance_Dataform__c newLead = leadMap.get(lead.Month__c);
        newLead.Month__c.addError('A Finance Data Form with this Month'
                               + ' already exists.');
        Finance_Dataform__c newLead2 = leadMap.get(lead.Year__c);
