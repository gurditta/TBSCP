trigger HR_DataformFieldPreventer on HR_Dataform__c (before insert, before update) {
     Map<String, HR_Dataform__c > leadMap = new Map<String, HR_Dataform__c >();
    for (HR_Dataform__c  lead : System.Trigger.new) {          
    ...
    for (HR_Dataform__c  lead : [SELECT Month__c, Year__c FROM HR_Dataform__c  
                      WHERE Month__c IN :leadMap.KeySet() and Year__c IN :leadMap.KeySet()]) {
        HR_Dataform__c  newLead = leadMap.get(lead.Month__c);
        newLead.Month__c.addError('A HR_Dataform Data Form with this Month'
                               + ' already exists.');
        HR_Dataform__c  newLead2 = leadMap.get(lead.Year__c);
